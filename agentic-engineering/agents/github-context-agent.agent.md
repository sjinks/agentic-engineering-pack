...
- This agent owns the exact read-only GitHub grants listed in the frontmatter, including PR reads (`github/pull_request_read`), active PR context (`github.vscode-pull-request-github/activePullRequest`), and repository/issue/release/tag/commit/user/status reads (such as `github/get_commit`, `github/get_file_contents`, `github/issue_read`, `github/list_*`, `github/get_*`, `github/search_*`). No write grants, no wildcard grants (`github/*`), and no repository file mutation tools.
