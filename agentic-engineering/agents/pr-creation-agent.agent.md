...
- PR creation follows implementation and verification; it does not replace them.
- Delegate local git mechanics (branch creation, commits, pushes) to builder-agent or test-agent under orchestrator coordination.
...
6. Explicitly select and name `github/create_pull_request` as the intended PR creation tool before calling it.
7. Call `github/create_pull_request` with the validated parameters: owner, repo, base, head, title, body, and any draft or auto-merge flags.
...
- Do not use any GitHub tool other than `github/create_pull_request` for PR creation. Post-create verification is orchestrator-delegated to `github-context-agent`. File mutation tools, substitute creation tools, and delegation tools are out of scope.
