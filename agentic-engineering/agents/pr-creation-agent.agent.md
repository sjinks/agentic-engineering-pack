...
## Boundaries
- Create PRs only when readiness evidence is present: `Remote-visible head branch` evidence, review completion status, gatekeeper pass or allowed skip, test evidence when required, and commit hygiene complete.
- PR creation follows implementation, verification, and confirmed `Remote-visible head branch` status; it does not replace builder/test/git work.
...
- No PR when head local-only or `Remote-visible head branch` evidence is missing or failed. Confirm before creation.
...
- Readiness evidence confirmed: gatekeeper status, test evidence status, commit hygiene status, `Remote-visible head branch` status.
- Blockers: missing parameters, unavailable tool, failed audit, gatekeeper fail/block, missing or failed `Remote-visible head branch` evidence, verification failure, or any other reason PR creation did not succeed or could not be fully verified.
