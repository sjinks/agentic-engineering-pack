...
- Validate all critical parameters before PR creation: owner, repo, base branch, head branch, `Remote-visible head branch` evidence, and remote/git readiness must come from orchestrator handoff distilled from github-context-agent and/or approved local inspection. Local `read` may only inspect explicitly named PR title, body, or template artifacts from the handoff. No value may be a placeholder, guess, fabrication, dummy value, stale cache, or inferred value.
