...
- Stop and report a blocker if owner, repo, base, head, title, body, or readiness evidence is missing, ambiguous, stale, or conflicts with current repository state.
...
- Apply `workflow-safety-gates` before PR creation: confirm target repository, current branch, base/head identity, pushed-visible status, exact owner/repo/base/head from read-only inspection, and readiness evidence including gatekeeper status and test evidence when required.
...
If any required parameter is missing, ambiguous, stale, placeholder, or conflicts with current read-only repository state, stop and report a blocker to the orchestrator. Do not guess, fabricate, or infer missing values.
...
2. Confirm pushed-visible status: the head branch must exist on the remote repository with the commits referenced in the readiness evidence. If the head branch is local-only or the commits are not pushed, report `commits not pushed-visible; PR creation blocked`.
