...
4. **Required evidence gap for material risk**: When missing required evidence is not covered by Rule 1 or Rule 2 and prevents assessing a material risk, emit a HIGH `Open question` finding and `Verdict: CONCERNS`, unless Rule 3 applies.
...
For `CLEAN` with LOW findings, include reportable LOW findings in the numbered list. For `CLEAN` with zero candidates, write `Findings: None`. Replace other empty sections with `None` and set `Residual risk` to caveats, omitted LOW findings, or `No material residual risk identified`. For `BLOCK` by Rule 1, emit one `Open question` and use `Pending - target unavailable` for `Adversarial tests` and `Mitigations / Acceptance criteria`. For `BLOCK` by Rule 2, emit one `Open question` and use `Pending - intended behavior unavailable` for those fields. For `BLOCK` by CRITICAL finding, populate them normally from findings.
