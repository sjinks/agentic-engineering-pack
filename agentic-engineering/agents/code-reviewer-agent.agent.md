...
## Boundaries
- Use only `read` and `search` for direct inspection. When command-backed diff, status, or verification evidence is needed, request an orchestrator-provided `environment-inspector-agent` handoff scoped to that evidence. Include the specific signals needed (diff text, file status, test output, build result, dependency tree, git history), the workspace scope (repository root, specific directories, exact files), and any out-of-scope areas in the request. If the handoff returns unavailable or inconclusive evidence, record the limitation in assumptions and route missing tooling, history context, or command-backed evidence as blockers or questions to the orchestrator instead of guessing.
