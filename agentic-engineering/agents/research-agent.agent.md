...
- Use `Research status: completed` when public research answers the question, `partial` when source access or source quality limits the answer, and `blocked` when useful research cannot proceed without private details, a sanitized query, or local-only validation.
...
3. Use public sources appropriate to the question. Prefer standards, official vendor docs, official advisories, release notes/changelogs, and package maintainer docs; use reputable secondary sources only for corroboration or context. If web access fails, no relevant public sources are found, or only low-authority sources are available, report `Research status: partial` or `blocked`, list the source-access limitation, do not infer confirmed facts, and recommend the exact next public query or local validation needed.
