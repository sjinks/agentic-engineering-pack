...
- Delegate implementation only for comments classified `valid/actionable` or valid portion of `partially valid`.
- Pass spec/design contract status, validated comment evidence, non-goals, affected files, exact mutation/check boundaries into Builder/Test handoffs.
- For reviewer-derived fixes, include equivalence-class audit instruction when applicable and require numeric `audited: N, deferred: M` reporting.
- Tests owned by `test-agent` unless handoff explicitly assigns bounded test-edit scope to Builder.
...
Broad safe validation is the broadest bounded, non-mutating, locally supported validation relevant to the changed surface, selected from repository-local evidence and changed-surface risk after targeted PR-review fix verification has already passed. Bounded validation is scoped to the repository, uses tools/configuration already present, and does not install packages, contact external systems, start services, mutate git state, or write unapproved output beyond ordinary test output. Its evidence is valid only when fresh for the final candidate worktree/fix batch.
...
This table identifies which status applies based on observable conditions.

| Observable Condition | Status |
| --- | --- |
| Selected broad safe validation completed successfully, dirty-state boundaries remained acceptable, and evidence is fresh for the final candidate worktree/fix batch. | `passed` |
| Selected broad safe validation ran and failed. | `failed` |
| Broad safe validation should run but cannot be selected or executed under current policy, tooling, dirty-state, or command-boundary constraints. | `blocked` |
| Broad safe validation is available but intentionally skipped with repository-local discovery evidence, candidate command(s) inspected, selected command or unavailable-command conclusion, command classification basis, freshness evidence for the final candidate worktree/fix batch, proceed/block effect, residual risk, and next operator action; workflow policy accepts the residual risk. | `skipped` |
| No meaningful broad validation exists for the changed surface after repository-local inspection; proceed only with repository-local discovery evidence, candidate command(s) inspected, selected command or unavailable-command conclusion, command classification basis, freshness evidence for the final candidate worktree/fix batch, proceed/block effect, residual risk, and next operator action. | `not applicable` |
| Only mutating, network, service-starting, package-management, or output-writing candidates exist; not a pass; proceed only with freshness evidence for the final candidate worktree/fix batch and after either the authorized mutating/output-writing candidate ran (reported separately with dirty-state/output boundaries) or an accepted residual-risk rationale explicitly covers not running it. | `mutating-only` |
...
- Select the broadest bounded candidate that is relevant to the changed surface and still safe under the current workflow boundaries. **Preference hierarchy for "broadest bounded":** full non-mutating relevant suite > changed-surface subset > static checks, selecting next-broadest if higher candidate is unavailable/mutating/approval-bound without approval. This does not mean "run the whole suite" by default; it means the widest safe local validation that reasonably exercises the affected surface without installing packages, contacting external systems, starting services, mutating git state, or writing unapproved output.
...
**Status Handling Table (explains consequences):**

This table defines what each status means and whether it blocks push/replies/resolution.

| Status | Meaning | Blocks push? |
|--------|---------|-------------| 
| `passed` | Selected broad safe validation completed successfully, dirty-state boundaries remained acceptable, and the evidence is fresh for the final candidate worktree/fix batch. | No |
| `failed` | Selected broad safe validation ran and failed. This blocks push, reviewer-facing replies, and thread resolution until the selected broad safe validation failure is addressed, or until the workflow is re-scoped or reclassified so that command is no longer the selected broad safe validation. A failed selected broad safe validation cannot be waived through residual risk. | Yes |
| `blocked` | Broad safe validation should run but cannot be selected or executed under current policy, tooling, dirty-state, or command-boundary constraints. | Yes |
| `skipped` | Broad safe validation is available but intentionally skipped only with repository-local discovery evidence, candidate command(s) inspected, selected command or unavailable-command conclusion, command classification basis, freshness evidence for the final candidate worktree/fix batch, proceed/block effect, residual risk, and next operator action. A skip is valid only when the workflow policy accepts the residual risk for this changed surface. | No (with rationale) |
| `not applicable` | No meaningful broad validation exists for the changed surface after repository-local inspection. This may proceed only with repository-local discovery evidence, candidate command(s) inspected, selected command or unavailable-command conclusion, command classification basis, freshness evidence for the final candidate worktree/fix batch, proceed/block effect, residual risk, and next operator action. | No (with evidence) |
| `mutating-only` | Only mutating, network, service-starting, package-management, or output-writing candidates exist. This is not a pass. It may proceed only with freshness evidence for the final candidate worktree/fix batch and after either the authorized mutating/output-writing candidate actually ran and is reported separately with dirty-state/output boundaries, or an accepted residual-risk rationale explicitly covers not running it. | Yes (until authorized mutation runs or risk accepted) |

**Status vs. Hard Gate Conditions:**

- **Missing status:** The Broad Safe Validation Gate was not consulted. Before proceeding, determine a status using the Status Decision Table above.
- **Stale / not fresh status:** Previously passing broad validation evidence was invalidated by subsequent relevant edits (contextual/independent review follow-up, builder/test edits, formatting, generated-output handling, or other worktree changes). Rerun validation or explicitly re-establish freshness and then determine a fresh status.
- **Push permission:** `passed` with fresh evidence permits push; `skipped` and `not applicable` permit push only with required evidence/rationale; `failed`, `blocked`, and unresolved `mutating-only`, plus missing/stale meta-conditions, block.
...
- Do not push, do not post reviewer-facing replies, and do not resolve threads if: (a) the Broad Safe Validation Gate was not consulted or status is missing, (b) evidence is stale or not fresh for the final candidate worktree/fix batch, or (c) status is `failed`, `blocked`, or `mutating-only` without authorized run or accepted risk.
...
- Read-only reviewer, security, adversary, environment-inspector, integrator agents must not commit, push, or perform local git mechanics.
