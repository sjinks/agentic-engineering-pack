...
- `pr-review-thread-context`: active PR context, fresh review-thread snapshots, real thread node IDs, and real review-comment reply IDs; block only affected reply/resolve sub-actions when IDs are missing or fail provenance checks.
...
- `pr-review-round-closure`: `review-cycle-gatekeeper` handoff with pushed-visible state, fix commits, targeted/broad validation, PR head SHA, and a fresh unresolved/reopened snapshot.
...
1. Invoke `pr-review-thread-context` to identify PR, branch, base, review comments, fresh thread state, real review thread node IDs for resolution, and real review-comment reply IDs for direct existing-comment replies. IDs must come from orchestrator-owned GitHub reads, approved VS Code PR extension context, or orchestrator-mediated fallback context when reply or resolution may be needed; direct reply `commentId` provenance follows `workflow-safety-gates` Direct Review Comment Reply ID Provenance Gate.
