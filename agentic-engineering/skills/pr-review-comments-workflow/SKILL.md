...
- `pr-review-fix-cycle`: Builder/Test handoff, targeted verification, Broad Safe Validation Gate, commit hygiene, Conventional Commit/body readiness, push, local push/ref evidence, and pushed-visible confirmation through GitHub visibility evidence.
...
Use `pr-review-fix-cycle` as the canonical commit, push, and local push/ref evidence contract. It covers current branch checks, default/base push denial, Local Git Mutation Delegation Contract, commit hygiene, Conventional Commit subject/body readiness, and scoped staging; final pushed-visible confirmation requires GitHub read evidence from github-context-agent.
