...
- `pr-review-thread-context`: active PR context, fresh review-thread snapshots, real thread node IDs, and real review-comment reply IDs sourced from `github-context-agent`; block only affected reply/resolve sub-actions when IDs are missing or fail provenance checks.
...
GitHub context and mutation are delegated to role agents, constrained by `workflow-safety-gates` allowlists: `github-context-agent` owns reads and Round-N, `pr-creation-agent` owns PR creation, and `pr-review-agent` owns direct replies and thread resolution. Specialists receive distilled PR data from orchestrator handoffs and must not be granted broad GitHub access. Direct invocation is valid only when the operator provides orchestrator-mediated PR context sourced from `github-context-agent`; without that context, stop and route the operator through the orchestrator instead of implying the skill can fetch PR data through direct GitHub access. Apply `workflow-safety-gates` before any reply, review/status mutation, thread resolution, branch/git mutation, or PR action. GitHub repository file mutation tools remain denied; all fixes go through local Builder/Test edit-and-push mechanics.
...
7. **Receive distilled Round-N and first-round risk context from the orchestrator handoff.** Before commit hygiene (step 8), the orchestrator passes the Round-N count sourced from `github-context-agent` reads (per the `workflow-safety-gates` Glossary entry "Round-N count"), the source citation (the `mcp_github_pull_request_read method=get_reviews` read backing the count, computed by `github-context-agent`, with the per-state breakdown of `APPROVED`, `CHANGES_REQUESTED`, and `COMMENTED` reviews enumerated per the Glossary state allowlist), the non-trivial by risk shape assessment for the cumulative branch diff vs the integration branch, Matched non-trivial class(es), Skip considered, Skip rejected evidence, Skip accepted evidence, and the Round-N-metadata-unreadable sentinel state. PR-author identity is NOT part of the round count; the round-N rule applies uniformly to all PRs, including bot-opened branches (Dependabot, Renovate, etc.). Conversation-tab comments are explicitly NOT counted (the metric is structured-review-only by design per the Glossary entry). Bot reviewer types (SAST integrations, Copilot, etc.) ARE counted. When Round-N count is `1`, a non-trivial cumulative branch diff triggers First-round non-trivial pre-push adversarial-review before the PR push proceeds. If the orchestrator handoff includes/provides the `Round-N-metadata-unreadable sentinel` after the Glossary retry/failure policy is exhausted, do NOT default to round 1; stop with a blocked status, surface the sentinel verbatim under the `Blockers` and `Pre-push adversarial review status` bullets in `## Output Format`, and ask the operator to re-fetch PR review state through `github-context-agent`. When this skill is invoked without the orchestrator on the call path (no orchestrator-mediated GitHub context available), treat round count and risk-shape trigger status as unknown and stop before gatekeeper, reply, or resolution.
