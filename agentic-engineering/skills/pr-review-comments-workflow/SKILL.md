...
- `pr-review-fix-cycle`: Builder/Test handoff, targeted verification, Broad Safe Validation Gate, commit hygiene, Conventional Commit/body readiness, push, local push/ref evidence, and orchestrator-sourced pushed-visible PR-diff confirmation.
...
| --- | --- | --- |
| **Gatekeeper-skip sentinel** | Gatekeeper was not invoked because: (a) no reviewer specialists ran for this change, or (b) reviewer specialists ran but produced no actionable findings requiring a fix cycle, per the `workflow-safety-gates` Glossary. | Gatekeeper run when a fix cycle occurred and findings need reconciliation. Targeted verification and Broad Safe Validation Gate evidence before push. No reviewer-facing replies or thread resolution while `fail` or `BLOCK` is unaddressed. |
| **Pre-push trivial skip** | Cumulative branch diff is eligible for skip only when the orchestrator reports a valid skip under `workflow-safety-gates` Glossary entries "Non-trivial by risk shape" and "Pre-push-adversary-skip sentinel"; non-trivial by risk shape wins over any skip consideration. | Adversarial review for any diff matching a non-trivial class. Adversarial review on first-round with a non-trivial diff. Targeted verification and Broad Safe Validation Gate evidence. |
...
Approved read-primary ID source: `mcp_github_pull_request_read` with method `get_review_comments`. If the approved read returns no threads, returns threads without the required IDs, or fails, treat thread/comment IDs as unavailable; block only the affected reply or resolve sub-action; report the missing ID/blocker in the Workflow Output Format; and do not use mutating tools as probes. Do not use a generic GraphQL CLI, generic API CLI, remote execute, or execute-capable path to recover missing IDs.

GitHub context and mutation are delegated to github-context-agent for reads and pr-review-agent for writes. Specialists receive distilled PR data from orchestrator handoffs sourced from github-context-agent and must not be granted broad GitHub access. The orchestrator calls github-context-agent for Round-N computation, PR metadata, and review-thread snapshots, then passes distilled context to pr-review-agent for write operations. Direct invocation is valid only when the operator provides orchestrator-mediated PR context; without that context, stop and route the operator through the orchestrator instead of implying the skill can fetch PR data through direct GitHub access. Apply `workflow-safety-gates` before any reply, review/status mutation, thread resolution, branch/git mutation, or PR action. GitHub repository file mutation tools remain denied; fixes go through local Builder/Test edits plus `git-operator-agent` branch/commit/push mechanics.
...
9. **Commit and push to the PR branch and confirm PR visibility.** Commit and push as part of this workflow unless the user explicitly says not to push, but only after the orchestrator reports `Pre-push adversarial review status` with `Execution status` and `Verdict` for any mandatory Round-N or first-round non-trivial trigger. The push may proceed only with completed execution and a non-blocking verdict, a valid trivial skip, or true not-applicable evidence; a `blocked` execution status or `Verdict: BLOCK` blocks the push. `git-operator-agent` returns local push/ref evidence only. Then confirm the pushed commits are visible in the PR through orchestrator-sourced `github-context-agent` PR-diff evidence before any reviewer-facing reply or thread-resolution action. If branch/upstream is ambiguous, ask before pushing. The orchestrator does not perform local git mechanics directly; commit and push mechanics must be delegated only to `git-operator-agent` under explicit workflow or user authorization, after branch and upstream checks and the Local Git Mutation Delegation Contract are recorded.
...
- Workflows use only direct existing-comment replies. Pending-review inline comments are outside the active reply surface.
...
Use `pr-review-fix-cycle` as the canonical commit, push, and PR visibility contract. It covers current branch checks, default/base push denial, Local Git Mutation Delegation Contract, commit hygiene, Conventional Commit subject/body readiness, scoped staging, local push/ref evidence, and orchestrator-sourced pushed-visible PR-diff confirmation.
...
Direct existing-comment replies require a numeric `commentId` with provenance accepted by `workflow-safety-gates` Direct Review Comment Reply ID Provenance Gate. Do not compose replies on any other review-comment surface from this workflow.
...
Pending-review inline comments are outside the active reply surface and are not composed by any active workflow.
