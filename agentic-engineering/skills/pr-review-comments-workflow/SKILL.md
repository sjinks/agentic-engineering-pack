...
- `pr-review-thread-context`: active PR context, fresh review-thread snapshots, and real thread/comment IDs; block only affected reply/resolve sub-actions when IDs are missing.
...
1. Invoke `pr-review-thread-context` to identify PR, branch, base, review comments, fresh thread state, and real review thread/comment IDs returned by orchestrator-owned GitHub reads, approved VS Code PR extension context, or orchestrator-mediated fallback context when reply or resolution may be needed.
