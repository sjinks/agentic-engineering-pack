...
- GitHub MCP context (PR metadata, review comments, thread state, commit/push visibility) stays orchestrator-only per `workflow-safety-gates`. This skill consumes distilled findings, fix evidence, and visibility status passed in by the operator or orchestrator; it does not call `github/*` itself.
