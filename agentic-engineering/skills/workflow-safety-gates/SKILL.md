...
## Shared Module Advisory Prompt Gate

Use this gate when the orchestrator's `## New Shared Module Prompt` stub is in scope. This is an advisory prompt, not a lexical enforcement gate; it forces an operator-visible decision for the common dependency-replacement bug class.

### Trigger

Surface the prompt before any push that introduces, or carries forward without a prior durable decision, a cumulative branch diff that matches BOTH conditions:

- The diff adds (`A`) or content-changing-renames (`R` below 100 similarity, preserving the raw `R100`/`R070` style status) at least one source file referenced by an added import/include/require line in another changed file in the same cumulative range. Pure `R100` renames do not satisfy this condition.
- The diff removes a runtime dependency from a declared-dependency manifest such as `package.json` dependencies/peerDependencies/optionalDependencies, `Cargo.toml`, `go.mod`, `pyproject.toml`, `Gemfile`, `composer.json`, `pom.xml`, or `pubspec.yaml`. Lockfile churn alone does not satisfy this condition.

Synthesis-based diffs are out of scope. In-place rewrites of existing modules that absorb a removed dependency's responsibilities are a documented residual false-negative; operators may still request `adversary-agent` plus `test-gap-to-test-plan` manually.

### Diff Evaluation Delegation

Delegate integration-branch resolution and cumulative-diff evaluation to `environment-inspector-agent`. The inspector must stay local-only, validate any operator-specified integration branch as an existing local ref, preserve rename similarity scores, return the resolved integration branch, cumulative file-status list, relevant added import/include lines, and removed manifest lines. Inconclusive evaluation emits the sentinel step `cumulative-diff`.

Evaluate failure steps in order: `integration-branch`, then `cumulative-diff`, then `host-capability`. On the first failure, emit the exact sentinel once in the visible handoff log and once in Output Format: `shared-module prompt cannot evaluate: <step> — <reason>; operator action required out-of-band`.

### Operator Choices

Use `vscode/askQuestions` when available. If unavailable but the host has an interactive commentary surface, prompt there and wait for the in-session response. If no interactive surface exists, stop with the sentinel step `host-capability`; never default to `decline` or `not applicable`.

Prompt content names the new file path(s), removed dependency name(s), and asks for one decision per file:

- `invoke`: run `adversary-agent` and `test-gap-to-test-plan` against the cumulative diff before push. Either unavailable/failed specialist, `BLOCK`, or unrouted must-have planner cases blocks push/PR creation. Findings from this path feed later equivalence-class audit handoffs.
- `decline`: skip the advisory pre-push adversary plus test-gap pass; require a one-line rationale.
- `not applicable`: record that the syntactic match is outside the targeted bug class, such as generated artifact, test helper, or configuration-only file; require a one-line reason.

When multiple files match, enumerate all files. Per-file decisions are required unless the operator explicitly applies the same decision and one rationale to all listed files.

### Rationale Validation And Durable Artifact

Decline and not-applicable rationales must be substantive: at least ten non-whitespace characters, not a single token such as `ok`, `nvm`, `wfh`, or `n/a`, and free of secrets, internal URLs/hostnames, customer/personal data, vault or memory paths, and workflow-internal tooling names when the text will be reviewer-facing. Re-prompt on rejection; if no passing rationale can be recorded, block with the sentinel step `host-capability` or the most specific available blocker.

Record the operator-facing decision in the visible handoff log and Output Format. For PR creation or PR-ready body preparation, record only reviewer-facing wording in the PR body under the PR Body Audience, Externally-Posted Content Gate, and PR Body Audit Gate. The durable branch record is a PR-body line for each decided file; before PR creation, an in-session decision must either be carried into same-session PR preparation or backed by another operator-provided durable record that can be verified before mutation. Without a durable record in a later session, re-fire before push.

### Residual Risk

This gate is advisory. Repeated declines without substantive rationale are a workflow-pattern signal for repository security/platform owners to review at the repository's supply-chain audit cadence, default quarterly when no cadence is documented. Shared-module decline does not bypass the separate First-round non-trivial pre-push adversarial-review rule.
...
The VS Code GitHub Pull Requests extension active-PR surface is approved for read-only PR context acquisition by `github-context-agent`:
...
- GitHub MCP read paths, including review comment reads such as `get_review_comments`, remain valid only through `github-context-agent`; this extension read surface does not replace the MCP paths or loosen specialist restrictions.
- Direct invocation of a PR workflow without orchestrator-mediated GitHub context from `github-context-agent` and without an approved active-PR read result must block or route through orchestrator-mediated context acquisition. Do not imply a direct skill or non-GitHub specialist can fetch PR/comments through `github/*` when that grant is unavailable.
...
- `linear/*` MCP tools are orchestrator-only in this pack. Broad `github/*` is not granted to the orchestrator or to specialists.
- GitHub authority is split across exact role agents: `github-context-agent` owns read-only GitHub PR/context/Round-N/thread reads; `pr-creation-agent` owns PR creation through the exact PR creation grant; `pr-review-agent` owns review replies and review-thread resolution through exact review-write grants.
- Specialists other than `github-context-agent`, `pr-creation-agent`, and `pr-review-agent` must not be granted direct `linear/*`, `github/*`, or GitHub exact-tool access unless future tooling updates the pack intentionally.
- The orchestrator coordinates GitHub workflows by delegating reads to `github-context-agent`, PR creation to `pr-creation-agent`, and review writes/thread resolution to `pr-review-agent`, then passing distilled context in handoffs. Other specialists do not self-service GitHub context.
- `github/pull_request_review_write` exposes multiple mutation methods at the tool layer. `pr-review-agent` must use only the methods approved by the GitHub Remote Mutation Allowlist and must apply Mutation Intent Gate, Critical Tool Parameter Gate, Untrusted External Content, Externally-Posted Content Gate, and Direct Review Comment Reply ID Provenance Gate before any write.
...
This audience rule applies to every PR-creation and PR-body-composition path in this pack, including direct invocation through `linear-issue-workflow`, delegated PR creation through `pr-creation-agent`, and the `pull-request-description` skill. Skills and agents that compose a PR body must inherit it; they do not need to repeat the full rule but they must not contradict it.
...
Before a workflow sends a body to `mcp_github_create_pull_request`, publishes a PR-ready body for manual creation, or returns a final fenced copy/paste PR description, apply this canonical PR Body Audit Gate. This gate applies to every PR-body path in the pack, including delegated `pr-creation-agent` PR creation, direct `linear-issue-workflow` invocation, and `pull-request-description`. The `pr-description-body-audit` support skill implements this gate for the composer; workflow paths apply the same checklist before creation or publication.
