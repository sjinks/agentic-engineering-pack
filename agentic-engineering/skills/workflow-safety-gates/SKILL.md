...
- Exact GitHub tools are granted with explicit read-write separation to three specialists: `github-context-agent` for all read-only operations (exact read-only grants listed in its frontmatter, including PR reads via `github/pull_request_read` and `github.vscode-pull-request-github/activePullRequest`, plus repository/issue/release/tag/commit/user/status reads such as `github/get_commit`, `github/get_file_contents`, `github/issue_read`, `github/list_*`, `github/get_*`, `github/search_*`; no write grants, no `github/*` wildcard, no repository file mutation tools), `pr-creation-agent` for PR creation only (`github/create_pull_request`), and `pr-review-agent` for review write operations only (`github/pull_request_review_write`, `github/add_reply_to_pull_request_comment`).
