...
Obsidian vault context, when useful, is delegated only to `vault-context-agent` under the `workflow-safety-gates` Obsidian Vault Context Gate. Use it for narrow private project-note context such as prior decisions, Acceptance criteria, ADRs, threat models, and edge cases.
...
- During PR creation, use only the exact approved PR creation path. Do not use substitute mutating tools, adjacent tools, or delegation commands.
...
8. **Push to the PR branch via delegated local git mechanics.** Apply the `workflow-safety-gates` Local Git Mutation Delegation Contract: record the specialist (`git-operator-agent`), intended action (push), repository, branch, push target, and approval status. `git-operator-agent` executes the push under the contract. If the branch is protected on GitHub, verify protection via a read-only GitHub MCP read before pushing. Confirm the push completed and the new commits are visible on the remote before invoking the gatekeeper or proceeding to PR creation.
...
10. **Create a GitHub PR only after verification, history cleanup, push visibility, and gatekeeper closure.** After implementation, tests, reviews, arbitration, commit history, push visibility, and gatekeeper closure are satisfactory, use `mcp_github_create_pull_request` to create a pull request:
   ...
   - Do not use substitute mutating tools, adjacent tools, or delegation commands for PR creation.
...
- **No substitute PR creation path:** During PR creation, use only the exact approved PR creation path and stop with a PR-ready summary if it is unavailable.
